...
import { defaultsHandler } from '../commands.js';
...
describe('defaultsHandler', () => {
  ...
  describe('S-2: Get single default', () => {
    it('should show value and source for specific key', async () => {
      ...
      await defaultsHandler(['get', 'includeHeaders'], state);
    ...
    it('should treat unknown subcommand as key lookup', async () => {
      // When user runs "defaults includeHeaders" (shorthand)
      await defaultsHandler(['includeHeaders'], state);
  ...
  describe('S-3: Get unknown key error', () => {
    it('should show error with suggestion for typo', async () => {
      // When user runs "defaults get inclueHeaders" (typo)
      await defaultsHandler(['get', 'inclueHeaders'], state);
    ...
    it('should show valid keys for completely wrong input', async () => {
      // When user runs "defaults get foobar"
      await defaultsHandler(['get', 'foobar'], state);
  ...
  describe('S-4: Set session override (boolean)', () => {
    it('should set boolean session override', async () => {
      ...
      // When user runs "defaults set includeHeaders true"
      await defaultsHandler(['set', 'includeHeaders', 'true'], state);
    ...
    it('should accept various boolean formats', async () => {
      // Yes
      await defaultsHandler(['set', 'includeHeaders', 'yes'], state);
      expect(state.sessionDefaults?.includeHeaders).toBe(true);

      // No
      await defaultsHandler(['set', 'trace', 'no'], state);
      expect(state.sessionDefaults?.trace).toBe(false);

      // 1
      await defaultsHandler(['set', 'showSummary', '1'], state);
      expect(state.sessionDefaults?.showSummary).toBe(true);

      // 0
      await defaultsHandler(['set', 'hideBody', '0'], state);
  ...
  describe('S-5: Set session override (enum)', () => {
    it('should set outputMode session override', async () => {
      ...
      await defaultsHandler(['set', 'outputMode', 'json'], state);
  ...
  describe('S-6: Set invalid enum value', () => {
    it('should reject invalid outputMode', async () => {
      ...
      await defaultsHandler(['set', 'outputMode', 'yaml'], state);
    ...
    it('should reject invalid boolean value', async () => {
      // When user runs "defaults set includeHeaders maybe"
      await defaultsHandler(['set', 'includeHeaders', 'maybe'], state);
  ...
  describe('S-7: Reset single override', () => {
    it('should reset specific session override', async () => {
      ...
      await defaultsHandler(['reset', 'includeHeaders'], state);
  ...
  describe('S-8: Reset all overrides', () => {
    it('should clear all session overrides', async () => {
      ...
      await defaultsHandler(['reset'], state);
  ...
  describe('Usage errors', () => {
    it('should show usage for get without key', async () => {
      await defaultsHandler(['get'], state);
      expect(consola.warn).toHaveBeenCalledWith('Usage: defaults get <key>');
    });

    it('should show usage for set without value', async () => {
      await defaultsHandler(['set', 'includeHeaders'], state);
      expect(consola.warn).toHaveBeenCalledWith('Usage: defaults set <key> <value>');
    });

    it('should show error for unknown subcommand', async () => {
      await defaultsHandler(['unknown'], state);
  ...
  describe('S-10: Session override affects HTTP commands', () => {
    it('should store session defaults in state for HTTP commands to use', async () => {
      ...
      await defaultsHandler(['set', 'includeHeaders', 'true'], state);
      await defaultsHandler(['set', 'trace', 'true'], state);
      await defaultsHandler(['set', 'outputMode', 'json'], state);
  ...
  describe('S-15: Method-specific source tracking', () => {
    it('should show workspace.method source when applicable', async () => {
      ...
      // Note: defaults command shows general view, not method-specific
