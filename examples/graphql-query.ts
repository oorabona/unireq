 ...
 * Demonstrates GraphQL queries with variables and fragments
 * Shows how to use @unireq/graphql with any GraphQL API
 * Usage: pnpm example:graphql-query
 */
...
`,
  ...
  if (countriesResponse.data?.data?.countries) {
    const countries = countriesResponse.data.data.countries.slice(0, 5); // First 5
